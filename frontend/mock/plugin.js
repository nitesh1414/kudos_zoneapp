  ...
  return {
    ...
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        ...
        if (path === '/api/dashboard') return json(res, { ...db.dashboard, role: db.me.role, username: db.me.username, authenticated: true, can_edit: db.me.role === 'admin' })
        if (path === '/api/health') return json(res, db.health)
        if (path === '/api/stats/zones') return json(res, db.stats_zones)
