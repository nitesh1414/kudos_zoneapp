...
import {
  Activity, BarChart3, BookOpen, CalendarDays, CalendarRange, CandlestickChart, ChevronDown, DatabaseZap,
  Layers, LayoutDashboard, LogOut, Menu, PlayCircle, Plug, RefreshCw, Users, X,
} from 'lucide-react'
...
export const NAV = [
  ...
  },
  {
    ...
    items: [
      ...
      { to: '/admin/methodology', label: 'Strategy & definitions', icon: BookOpen },
    ],
  },
]
...
/** Every signed-in account can point the market tabs at any tracked symbol. */
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="sticky top-0 z-40 border-b border-white/8 bg-ink-950/80 backdrop-blur-xl">
          <div className="mx-auto flex w-full max-w-[1400px] items-center justify-between gap-3 px-4 py-3 sm:px-6">
            ...
            <div className="flex items-center gap-2.5">
              <SymbolSwitcher className="hidden md:flex" />
              <Button
                variant="ghost"
                size="sm"
                icon={RefreshCw}
                onClick={() => window.dispatchEvent(new Event('zoneapp:refresh'))}
        ...
        <main className="mx-auto w-full max-w-[1400px] flex-1 px-4 pt-4 pb-16 sm:px-6">
          <SymbolSwitcher className="mb-3 flex w-full md:hidden" />
