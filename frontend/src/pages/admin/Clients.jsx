...
const emptyForm = { display_name: '', username: '', password: '' }

function ClientForm({ mode, form, setForm, error }) {
  ...
  return (
    <div className="space-y-3.5">
      ...
      <p className="rounded-xl border border-white/10 bg-white/4 px-3.5 py-3 text-[12px] leading-relaxed text-slate-500">
        Clients are not tied to a symbol or a broker: every account can view any symbol on the watchlist using the
    ...
    <div className="card animate-rise flex flex-col gap-3 p-4 transition hover:border-brand-500/30">
      ...
      <dl className="space-y-1 text-[12.5px]">
        <div className="flex justify-between gap-3 border-b border-white/5 py-1.5">
          ...
          <dd className="truncate font-semibold text-slate-200">All tracked symbols</dd>
        </div>
...
export default function Clients() {
  ...
  const rows = useMemo(() => {
    ...
    return list.filter(
      (c) =>
        ...
        (!q || [c.display_name, c.username].some((v) => String(v || '').toLowerCase().includes(q))),
    )
  ...
  const openEdit = (c) => {
    setForm({ display_name: c.display_name || '', username: c.username, password: '' })
    setError('')
  ...
  const submit = async (e) => {
    ...
    try {
      if (modal.mode === 'create') {
        ...
        const body = { display_name: form.display_name.trim() }
  ...
  const stats = [
    ...
    { label: 'Disabled', value: all.filter((c) => !c.active).length, icon: KeyRound, tone: 'text-amber-400 bg-amber-500/10' },
  ]
  ...
  return (
    <div className="space-y-4">
      ...
      <Card
        ...
        icon={Users}
        subtitle="Separate logins for the same market data. Every client can view any tracked symbol."
        right={
      ...
      >
        <div className="mb-4 flex flex-wrap items-center gap-2">
          <div className="relative min-w-[12rem] flex-1">
            ...
            <Input className="pl-9" placeholder="Search name or username…" value={query} onChange={(e) => setQuery(e.target.value)} />
          </div>
            ...
            <tbody>
              {rows.map((c) => (
                <tr key={c.id} className="transition hover:bg-white/3">
                  ...
                  <Td className="text-xs text-slate-400">All tracked symbols</Td>
      ...
      >
        <form id="client-form" onSubmit={submit}>
          <ClientForm mode={modal?.mode} form={form} setForm={setForm} error={error} />
        </form>
      </Modal>
