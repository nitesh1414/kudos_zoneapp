...
/** Star ratings only render for administrators; clients get the base rates. */
export function ZoneRow({ zone, rate, showStars }) {
  ...
  return (
    <tr className={`transition ${kindRow[kind]}`}>
      ...
      <Td className="num text-slate-300">{fmtPct(zone.touch_pct ?? rate?.touch_pct)}</Td>
      <Td className="num text-slate-300">{fmtPct(zone.hold_pct ?? rate?.hold_pct)}</Td>
    </tr>
  )
}
          ...
          >
            {rows.length === 0 ? (
              ...
              <>
                ...
                <Note>
                  Reference map from the last completed session — not a trade signal or forecast. Touch rate is how
                  often a level of this quality was reached; held is how often it was defended once reached.
