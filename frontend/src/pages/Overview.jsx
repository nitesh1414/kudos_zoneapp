import {
  Activity, ArrowDownRight, ArrowUpRight, CalendarCheck, Database, Gauge, Layers, Radar, Server, Timer,
} from 'lucide-react'
...
function StatsRow() {
  const { data, loading } = useApi(endpoints.health)
  if (loading)
    return (
  ...
  return (
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      <Stat icon={Database} tone="brand" label="Bars in database" value={fmtNum(data?.bars)} hint="Raw intraday candles" />
      ...
      <Stat
        ...
        hint={data?.server_time ? fmtDate(data.server_time) : 'Asia/Kolkata'}
      />
    </div>
  )
}
...
function RecapCard({ recap, match }) {
    ...
    <div className="space-y-4">
      ...
      {data && (
        <>
          ...
          <RecapCard recap={data.session_recap} match={data.match_check} />
        </>
      )}
    </div>
  )
