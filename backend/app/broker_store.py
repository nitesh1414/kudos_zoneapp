...
def symbols_for(store, broker_id: int):
    """Every symbol that depends on this connection."""
    rows = store.q("""SELECT DISTINCT u.symbol FROM client_brokers cb JOIN users u ON u.id = cb.user_id
                      WHERE cb.broker_id = ? AND u.active = true""", [broker_id])
    symbols = [r["symbol"] for r in rows.to_dict("records")] if not rows.empty else []
