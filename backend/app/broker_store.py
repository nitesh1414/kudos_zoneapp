...
def symbols_for(store, broker_id: int):
    """Every symbol this connection must keep up to date: watchlist entries
    pinned to it (or to no connection at all) plus its clients' symbols."""
    rows = store.q("""SELECT DISTINCT u.symbol FROM client_brokers cb JOIN users u ON u.id = cb.user_id
                      WHERE cb.broker_id = ? AND u.active = true
                      UNION
                      SELECT symbol FROM tracked_symbols
                      WHERE active = true AND (broker_id IS NULL OR broker_id = ?)
                      ORDER BY 1""", [broker_id, broker_id])
