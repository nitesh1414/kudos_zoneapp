...
# Tracked automatically on first start, so a new installation has something to
...
def all_symbols(store):
    """Every symbol the platform tracks. Accounts are not tied to a symbol —
    the watchlist is the single source of truth."""
    rows = store.q("SELECT symbol FROM tracked_symbols WHERE active = true ORDER BY symbol")
